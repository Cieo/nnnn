...
struct ngx_module_s {
    ngx_uint_t            ctx_index;                            //在type类模块数组里的序号
    ngx_uint_t            index;                                //在所有模块数组里的序号

    char                 *name;                                 //模块的名字

    ngx_uint_t            spare0;                               //保留字段，暂无意义
    ngx_uint_t            spare1;                               //同上

    ngx_uint_t            version;                              //模块的版本，同nginx_version
    const char           *signature;                            //模块的签名信息

    void                 *ctx;                                  //特定模块的专用数据，通常是函数指针表
    ngx_command_t        *commands;                             //模块的指令数组，定义在配置文件里的指令
    ngx_uint_t            type;                                 //模块的类形标记

    ngx_int_t           (*init_master)(ngx_log_t *log);         //函数指针，类似成员函数

    ngx_int_t           (*init_module)(ngx_cycle_t *cycle);     //函数指针

    ngx_int_t           (*init_process)(ngx_cycle_t *cycle);    //函数指针
    ngx_int_t           (*init_thread)(ngx_cycle_t *cycle);     //函数指针
    void                (*exit_thread)(ngx_cycle_t *cycle);     //函数指针
    void                (*exit_process)(ngx_cycle_t *cycle);    //函数指针

    void                (*exit_master)(ngx_cycle_t *cycle);     //函数指针
